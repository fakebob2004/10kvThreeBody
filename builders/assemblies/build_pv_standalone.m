function modelFile=build_pv_standalone()
%BUILD_PV_STANDALONE Generate the v2 PV-GFL commissioning harness from zero.
root=setup_v2_paths();define_resource_buses(true);define_v2_parameters(true);
define_strong_machine_fixture(true);
model='pv_resource_standalone';folder=fullfile(root,'models','resources');
...
pv=build_pv_resource(model,'PV Resource','V2.PV',[300 175 760 455]);
add_command_profile(model,'PV Command',[40 220 220 400]);
add_grid_harness(model,pv);
add_status_recorder(model);
add_block('nesl_utility/Solver Configuration',[model '/Physical Network Solver'], ...
    'Position',[890 560 955 615]);
add_line(model,v2_port([model '/Physical Network Solver'],'RConn',1),v2_physical_port(pv));
add_line(model,'PV Command/1','PV Resource/1','autorouting','on');
add_line(model,'PV Resource/1','PV Status Recorder/1','autorouting','on');

a=Simulink.Annotation(model,sprintf([ ...
    'PV RESOURCE v2 — generated from empty model\n' ...
    'Resource: Source/DC -> shared GFL PCS -> independent Filter/Transformer/PCC\n' ...
    'Standalone harness: strong synchronous machine + local load; no BESS or Wind']));
...
set_param(model,'InitFcn',initCmd, ...
    'StopTime','1.0','MaxStep','1e-5','SolverType','Variable-step','Solver','ode23t', ...
    'Location',[50 50 1450 880]);
...
v2_render(model,fullfile(artifact,'PV_01_Standalone.png'));
v2_render(pv,fullfile(artifact,'PV_02_Resource.png'));
v2_render([pv '/02 PCS'],fullfile(artifact,'PV_03_GFL_PCS.png'));
v2_render([pv '/03 AC Interface'],fullfile(artifact,'PV_04_AC_Interface.png'));
close_system(model,0);fprintf('PV_V2_BUILD_PASS=1\nMODEL=%s\n',modelFile);
end
...
function add_grid_harness(model,pv)
root=setup_v2_paths();
grid=add_strong_machine_fixture(model,'Strong Synchronous Machine Test Grid',root,[880 165 1250 450]);
add_line(model,v2_physical_port(pv),v2_port(grid,'RConn',1),'autorouting','on');
end

function add_status_recorder(model)
s=[model '/PV Status Recorder'];add_block('built-in/Subsystem',s,'Position',[810 455 1080 535], ...
...
add_block('simulink/Signal Routing/Mux',[s '/Legacy Regression Vector'], ...
    'Position',[250 25 255 285],'Inputs','12');
add_line(s,'Time/1','Legacy Regression Vector/1');
for k=1:11,add_line(s,sprintf('Status Selector/%d',k),sprintf('Legacy Regression Vector/%d',k+1),'autorouting','on');end
add_line(s,'ResourceStatusBus/1','Status Selector/1');
add_block('simulink/Sinks/To Workspace',[s '/PV v2 Result'], ...
    'Position',[330 125 455 165],'VariableName','pv_v2_result','SaveFormat','Array','MaxDataPoints','inf');
add_line(s,'Legacy Regression Vector/1','PV v2 Result/1');v2_route_level(s);
end
