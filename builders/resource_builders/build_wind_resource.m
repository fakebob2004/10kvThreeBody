function path=build_wind_resource(parent,name,cfg,pos)
%BUILD_WIND_RESOURCE Generate an independent three-module Wind-GFL resource.
path=[parent '/' name];add_block('built-in/Subsystem',path,'Position',pos, ...
    'BackgroundColor','[0.88,0.94,1.00]','ContentPreviewEnabled','off', ...
    'AttributesFormatString','Wind Source/DC -> GFL PCS -> independent AC Interface');
...
add_wind_source_dc(path,'01 Wind Source and DC Side',cfg,[100 85 345 210]);
...
add_line(path,'ResourceCommandBus/1','01 Wind Source and DC Side/1','autorouting','on');
add_line(path,'01 Wind Source and DC Side/1','02 PCS/1','autorouting','on');
add_line(path,'03 AC Interface/1','02 PCS/2','autorouting','on');
add_line(path,'01 Wind Source and DC Side/2','Status Interface/1','autorouting','on');
