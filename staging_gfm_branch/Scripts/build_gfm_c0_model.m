%BUILD_GFM_C0_MODEL Add a readable controller to the independent GFM plant.
...
source='GFM_BESS_Plant_C0'; model='GFM_Inverter_C0';
sourceFile=fullfile(root,[source '.slx']); modelFile=fullfile(root,[model '.slx']);
assert(isfile(sourceFile), ...
    'Missing independent GFM plant. Run Scripts/build_gfm_plant_c0_model.m first.');
bdclose('all'); load_system(sourceFile); save_system(source,modelFile); close_system(source,0);
load_system(modelFile);
set_param(model,'Solver','ode15s','MaxStep','Control.PWM.MaxStep', ...
    'StopTime','GFM.StopTime','SignalLogging','on','SignalLoggingName','logsout');
...
old=[model '/Controller Placeholder'];
...
note(model,[600 420 1320 470], ...
    'Independent GFM plant: active detailed VSC | R-L filter | 0.69/10 kV transformer | SCR grid.');
...
set_param(model,'Description', ...
    'C0 readable grid-forming controller integrated into the independent GFM-BESS detailed switching plant.');
set_param(model,'Location',[30 70 1760 810],'ZoomFactor','FitSystem');
for n={'GatePulses','Vdc_Meas','Vabc_PCC_Meas','Iabc_PCC_Meas', ...
        'Vabc_inv','Vll_inv','Iabc_sw', ...
...
add_block('simulink/Math Operations/Abs',[sys '/Abs Current'], ...
    'Position',[300 285 350 315],'ZeroCross','off');
add_block('simulink/Math Operations/MinMax',[sys '/Peak Phase Current'], ...
    'Position',[400 280 455 320],'Function','max','Inputs','1','ZeroCross','off');
...
add_block('simulink/Math Operations/MinMax',[sys '/Safe Ipeak'], ...
    'Position',[105 270 165 310],'Function','max','Inputs','2','ZeroCross','off');
